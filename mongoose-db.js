...
User.find({ username: 'admin@khulnasoft.com.io' }).exec(function (err, users) {
  console.log(users);
  if (users.length === 0) {
    console.log('no admin');
    new User({ username: 'admin@khulnasoft.com.io', password: 'SuperSecretPassword' }).save(function (err, user, count) {
      if (err) {
